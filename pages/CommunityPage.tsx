...
const PostCard: React.FC<{ post: Post }> = ({ post }) => {
    ...
    return (
        <Link to={`/post/${post.id}`} className="block bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
        ...
        <div className="bg-slate-100 dark:bg-slate-900 min-h-screen animate-fade-in" dir="rtl">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div className="flex flex-col sm:flex-row justify-between items-center mb-10 gap-4">
                    <h1 className="text-4xl font-extrabold text-gray-800 dark:text-white">مجتمع هليوبوليس</h1>
                    <div className="flex items-center gap-4">
                        <select value={sortOrder} onChange={e => setSortOrder(e.target.value as 'latest' | 'popular')} className="bg-white dark:bg-slate-800 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-cyan-500">

...
const QuickAccessCard: React.FC<{ icon: React.ReactNode; title: string; to: string; }> = ({ icon, title, to }) => (
    <Link to={to} className="group flex flex-col items-center justify-center p-6 bg-white dark:bg-slate-800 rounded-xl shadow-lg text-center transform hover:-translate-y-2 transition-transform duration-300">
        <div className="flex items-center justify-center w-16 h-16 bg-cyan-100 dark:bg-cyan-900/50 rounded-full mb-4 transition-all duration-300 group-hover:scale-110 group-hover:bg-cyan-200">
            {icon}
        </div>
...
const PublicHomePage: React.FC = () => {
    const { publicPagesContent, services, categories, advertisements, news, properties } = useAppContext();
    ...
    return (
        <div className="animate-fade-in" dir="rtl">
            {/* Hero Section */}
            <section className="relative bg-slate-100 dark:bg-slate-900 pt-10 pb-12 md:pt-16 md:pb-20 text-center overflow-hidden">
                ...
                <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                    ...
                    <div className="max-w-xl mx-auto">
                        <div className="relative">
                           <MagnifyingGlassIcon className="w-6 h-6 text-gray-400 absolute top-1/2 right-4 -translate-y-1/2" />
                           <input type="search" placeholder="ابحث عن مطعم، صيدلية، أو أي خدمة..." className="w-full pl-4 pr-12 py-4 text-lg rounded-full shadow-lg focus:outline-none focus:ring-2 focus:ring-cyan-500 transition"/>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section className="py-16">
                 <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                    <h2 className="text-3xl font-bold text-center text-gray-800 dark:text-white mb-10">{content.featuresSectionTitle}</h2>
            ...
            <section className="py-16 bg-slate-50 dark:bg-slate-900/50">
                 <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                    <h2 className="text-3xl font-bold text-center text-gray-800 dark:text-white mb-10">أحدث الخدمات المضافة</h2>
            ...
            {recentProperties.length > 0 && (
                <section className="py-16 bg-white dark:bg-slate-900">
                    <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                        <h2 className="text-3xl font-bold text-center text-gray-800 dark:text-white mb-10">أحدث العقارات</h2>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            {/* Recent News Section */}
             {recentNews.length > 0 && (
                <section className="py-16 bg-slate-50 dark:bg-slate-900/50">
                    <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                        <h2 className="text-3xl font-bold text-center text-gray-800 dark:text-white mb-10">آخر الأخبار</h2>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
